...
function isValidRRN(rijksregisternummer) {
    var regex = new RegExp(/\d{6}-\d{3}-\d{2}/);
    return regex.test(rijksregisterNummer);
}

function isValidDate(rijksregisterNummer){
    var regex = new RegExp(/^\d{2}(0[1-9]|1[1-2])(0[1-9]|[1-3][0-9])/);
    return regex.test(rijksregisterNummer);
}
...
console.log(isValidRRN(rijksregisterNummer));
console.log(isValidDate(rijksregisterNummer));
